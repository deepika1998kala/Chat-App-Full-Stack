...
const ChatContainer = () => {
  ...
  const handleSendImage = async (e) =>{
    ...
    if(!file || !file.type.startsWith("image/")){
      toast.erro("Select an image file")
      return;
    }
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      {/* Chat container 2nd column header */}
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        <img src={selectedUser.profilePic || img.avatar_icon} alt="" className='w-8 rounded-full' />
          ...
          >
            {msg.image ? (
              <img
                src={msg.image}
                alt=""
                className='max-w-[230px] border border-gray-200 rounded-lg overflow-hidden mb-8'
              />
            ) : (
              <p
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        ...
        <img onClick={handleSendMessage} src={img.send_button} alt="" className='w-7 cursor-pointer'/>

      </div>
    </div>
  ) : (
    <div className='flex flex-col items-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
      <img src={img.logo_icon} className='max-w-16' alt="" />
